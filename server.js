...
const PROVIDERS = {
  ...
  gemini: { baseURL: 'https://generativelanguage.googleapis.com/v1beta/openai', defaultModel: 'gemini-2.0-flash-001' },
};
...
app.post('/api/compile', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(502).json({ error: `LLM API error: ${data.error?.message || response.statusText}` });
    }
